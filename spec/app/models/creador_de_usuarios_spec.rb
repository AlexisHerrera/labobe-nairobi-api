...
describe 'CreadorDeUsuarios' do
  it 'deberia crear usuario y guardar en el repositorio' do
    ...
    expect(repo).to receive(:has).with("123").and_return(false)
    expect(repo).to receive(:has_telefono).with("1234567890").and_return(false)
  ...
  it 'no deberia guardar un numero que ya esta en uso' do
    ...
    expect(repo).to receive(:has).with("123").and_return(false)
    expect(repo).to receive(:has_telefono).with('1234567890').and_return(true)
